...
class CenterPointLightning(pl.LightningModule):
    ...
    def apply_modality_dropout(self, batch):
        ...
        # Independent dropout for each modality
        drop_radar = torch.rand(1, device=self.device).item() < self.drop_radar_prob
        drop_cam = torch.rand(1, device=self.device).item() < self.drop_cam_prob
        drop_feat = torch.rand(1, device=self.device).item() < self.drop_feat_prob

        # Never drop all modalities
        if drop_radar and drop_cam and drop_feat:
            # Keep the modality with lowest dropout probability
            probs = [self.drop_radar_prob, self.drop_cam_prob, self.drop_feat_prob]
