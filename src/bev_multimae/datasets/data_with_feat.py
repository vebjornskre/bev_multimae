...
def collate_radar(samples):
    ...
    if "bev_feat" in samples[0]:
        batch["bev_feat"] = torch.stack([s["bev_feat"] for s in samples])

    radar_list = [s["radar"] for s in samples]
    radar_batch = {}

    for k in radar_list[0].keys():
        if k in {"batch_size", "points", "pillar_coords", "pillar_inv"}:
            continue
...
class BEVDataset(Dataset):
    def __init__(
        ...
        self.meta = torch.load(os.path.join("data/processed/right", "meta.pt"), map_location="cpu", weights_only=False)
    ...
    def augment_sample(self, cam, bev_feat, rad_points):
        ...
        bev_target = build_bev_target(
            batch_dict,
            grid_size=self.grid_size[:2],
            num_rad_channels=self.num_rad_channels,
        )

        return cam, bev_feat, batch_dict, bev_target
    ...
    def __getitem__(self, idx):
        ...
        bev_feat = None
        if "bev_feat" in data:
            bev_feat = data["bev_feat"].float()

        if self.augment:
            cam, bev_feat, data["radar"], data["radar_target"] = self.augment_sample(
                cam,
                bev_feat,
                data["radar"]["points"],
            )
        ...
        if self.rad_mean is not None and self.rad_std is not None:
            data["radar"]["points"][:, 4:7] = (
                data["radar"]["points"][:, 4:7] - self.rad_mean
            ) / (self.rad_std + 1e-6)
