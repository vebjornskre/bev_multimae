...
def extract(cfg, input_path):
    ...
    with open(input_path, "rb") as f:
        ...
        for schema, channel, message in reader.iter_messages():
            ...
            if "points" in topic and "lidar" not in topic:
                with open(os.path.join(output_dir, f"{timestamp}.bin"), "wb") as f_bin:
                    f_bin.write(message.data)
                continue

            for _, _, _, ros_msg in reader.iter_decoded_messages(topics=[topic]):
                if topic.endswith("compressed_image"):
                    with open(os.path.join(output_dir, f"{timestamp}.jpg"), "wb") as f_img:
                        f_img.write(ros_msg.data)
                    break
                ...
                if "gt_seg" in topic:
                    ...
                    for ann in ros_msg.points:
                        pts = [(int(p.x), int(p.y)) for p in ann.points]
                        if len(pts) >= 3:
                ...
                if "lidar" in topic and "points" in topic:
