...
def run_detect(model, batch, point_cloud_range, cfg):
    with torch.no_grad():
        ...
        # Apply double-flip test-time augmentation if enabled
...
def setup_and_infer(cfg, sample_idx, visualize=True):
    ...
    if cfg.get("ablation_test", False):
        ...
        batch_no_cam = copy.deepcopy(batch)
        batch_no_cam["cam_bev"] = torch.zeros_like(batch_no_cam["cam_bev"])
        det_no_cam, dec_no_cam = run_detect(model, batch_no_cam, point_cloud_range, cfg)
        ablation["no_camera"] = {"detections": det_no_cam, "decoded": dec_no_cam}
        ...
        if "bev_feat" in batch:
            ...
            batch_no_cam_feat = copy.deepcopy(batch)
            batch_no_cam_feat["cam_bev"] = torch.zeros_like(batch_no_cam_feat["cam_bev"])
            batch_no_cam_feat["bev_feat"] = torch.zeros_like(batch_no_cam_feat["bev_feat"])
            det_no_cam_feat, dec_no_cam_feat = run_detect(model, batch_no_cam_feat, point_cloud_range, cfg)
            ablation["no_camera_no_bev_feat"] = {
                "detections": det_no_cam_feat,
                "decoded": dec_no_cam_feat,
            }
