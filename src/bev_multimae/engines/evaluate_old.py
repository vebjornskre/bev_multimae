...
def plot_ap_by_iou(cache, save_dir):
    ...
    plt.figure(figsize=(7, 5))
    plt.bar(["AP@10", "AP@20", "AP@30", "AP@50"], ap_values)
    plt.ylabel("AP")
...
def run_model(model, batch, point_cloud_range, cfg):
    ...
    # print(f"drop_rad_inference: {cfg.get('drop_rad_inference', False)}")
    # print(f"drop_feat_inference: {cfg.get('drop_feat_inference', False)}")
...
def collect_predictions(model, loader, device, point_cloud_range, cfg, decode_score_thresh):
    ...
    for batch in loader:
        ...
        for i, dec in enumerate(decoded):
            ...
            for pred in pred_boxes[:3]:
                if target_boxes:
                    best_3d = max(iou_3d(pred, target) for target in target_boxes)
                    best_bev = max(iou_bev(pred, target) for target in target_boxes)
                    # print("best IoU3D:", best_3d, "best IoU-BEV:", best_bev)
...
def main(cfg: DictConfig):
    ...
    with open(os.path.join(save_dir, "ap_iou50.txt"), "w") as f:
        f.write(f"{ap_iou50:.6f}\n")
