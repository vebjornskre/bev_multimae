...
def load_pretrain(encoder, path):
    ...
    log.info(f"Loaded pretrained encoder from {path}")
...
def run_finetune(cfg: DictConfig):
    ...
    if cfg.load_pretrained_encoder and cfg.pretrained_checkpoint and not cfg.continue_training:
        ckpt_path = os.path.join(cfg.model_folder, cfg.pretrained_checkpoint)
        ...
        encoder = load_pretrain(encoder, ckpt_path)
