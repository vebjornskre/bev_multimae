...
from bev_multimae.multimae.model_utils import (Block, CrossAttention, Mlp,
                             positional_encoding_2d, pair)
...
class SpatialOutputAdapter(nn.Module):
    ...
    def __init__(self,
        ...
        if not self.learnable_pos_emb:
            pe = positional_encoding_2d(h_posemb, w_posemb, self.dim_tokens)  # [H*W, D]
            pe = pe.view(h_posemb, w_posemb, self.dim_tokens)                 # [H, W, D]
            pe = pe.permute(2, 0, 1).unsqueeze(0)                             # [1, D, H, W]
            self.pos_emb = nn.Parameter(pe, requires_grad=False)
        else:
    ...
    def get_queries_and_context(self, context_tokens, input_info, ids_keep, ids_restore):
        ...
        H, W = input_info['tasks'][self.task]['recon_size'][:2]
